...
export function RevealForm({ action, children, className }: { action: (prev: RevealResult, fd: FormData) => Promise<RevealResult>; children: ReactNode; className?: string }) {
  const [state, formAction, pending] = useActionState(action, undefined);
  return (
    <div>
      <form action={formAction} className={className}>
        {children}
        <button className="btn btn-primary" disabled={pending}>{pending ? "Working..." : "Create"}</button>
      </form>
      {state && (
